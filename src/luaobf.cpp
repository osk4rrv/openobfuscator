...
std::string Obfuscator::buildLuaJitSourceVm(std::string_view source) {
    ...
    std::ostringstream ss;
    ss << "-- OpenObfuscator encoded loader format 1\n"
       << "return(function(...)\n"
       ...
       << "if " << vFormat << "~=1 then error(\"integrity:loader\",0)end\n"
       ...
       << "if " << vHalted << " then error(\"integrity:loader\",0)end\n"
       << "if " << vOp << "==" << opHalt << " then\n"
       << vHalted << "=true\n"
       ...
       << vKey << "=" << vKey << "\n"
       << "else\n"
       << "error(\"integrity:loader\",0)\n"
       << "end\n"
       << "end\n"
       << "if not " << vHalted << " then error(\"integrity:loader\",0)end\n"
       << "local " << vSrc << "=" << vConcat << "(" << vOut << ")\n"
       << "if #" << vSrc << "~=" << source.size() << " then error(\"integrity:loader\",0)end\n"
       ...
       << "if " << vAdler << "~=" << adler32(source) << " then error(\"integrity:loader\",0)end\n"
       << "local " << vLoad << "=loadstring or load\n"
       << "local " << vFn << "," << vErr << "=" << vLoad << "(" << vSrc << ",'@openobfuscator-loader')\n"
       << "if not " << vFn << " then error(\"load:loader \"..tostring(" << vErr << "),0)end\n"
       << "return " << vFn << "(...)\n"
       << "end)(...)\n";
    return ss.str();
}
...
std::string Obfuscator::buildJavaScriptSourceVm(std::string_view source) {
    ...
    std::ostringstream ss;
    ss << "// OpenObfuscator JavaScript encoded loader format 1\n"
       << "(function(){\n"
       << "const _format=" << FormatVersion << ";\n"
       << "if(_format!==1)throw new Error('integrity:loader');\n"
       ...
       << "if(" << vHalted << ")throw new Error('integrity:loader');\n"
       << "if(" << vOp << "===" << vm.opHalt << "){\n"
       << vHalted << "=true;\n"
       ...
       << vKey << "=" << vKey << ";\n"
       << "}else{throw new Error('integrity:loader');}\n"
       << "}\n"
       << "if(!" << vHalted << ")throw new Error('integrity:loader');\n"
       << "const " << vBytes << "=new Uint8Array(" << vOut << ");\n"
       << "if(" << vBytes << ".length!==" << source.size() << ")throw new Error('integrity:loader');\n"
       ...
       << "if(" << vAdler << "!==" << adler32(source) << ")throw new Error('integrity:loader');\n"
       << "let " << vSrc << ";\n"
